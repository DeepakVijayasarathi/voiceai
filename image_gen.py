...
def generate_story_image(story_text: str) -> bytes:
    ...
    a clean illustration, no baked-in text. (An earlier version asked the
    image model to render the title directly into the artwork; dropped -
    image-model text rendering, especially for non-Latin Indic scripts,
    came out garbled often enough not to be worth it. The title is shown
    reliably instead as an HTML/CSS scrim over the image in the UI - see
    ui/index.html's CoverArt component.) Raises ImageGenError on failure
    (missing key, network error, unexpected response) - this is opt-in,
    so the caller should see the failure rather than have it silently
    dropped."""
