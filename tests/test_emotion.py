from emotion import VALID_EMOTIONS, _normalize_item, _reconcile_length
...
def test_normalize_item_rejects_invalid_genre_and_sfx():
    ...
    assert result["sfx"] == "none"


def test_normalize_item_rejects_unknown_speaker():
    item = {"genre": "neutral", "sfx": "none", "speaker": "SomeoneNotInCast"}
...
def test_normalize_item_clamps_intensity_to_0_1():
    item = {"genre": "neutral", "sfx": "none", "bgm_intensity": 5.0, "sfx_intensity": -3.0}
    result = _normalize_item(item, known_characters=[])
...
def test_reconcile_length_more_items_than_expected():
    ...
    assert len(result) == 1
    # Should surface a real (non-fallback) genre/sfx rather than discarding everything.
    assert result[0]["genre"] in VALID_EMOTIONS
    assert result[0]["sfx"] in SFX_TYPES
    assert result[0]["sfx"] != "none"
...
def test_reconcile_length_empty_items_returns_defaults():
    ...
    assert len(result) == 2
    assert all(r["genre"] == "neutral" and r["sfx"] == "none" for r in result)


def test_reconcile_length_preserves_speaker_attribution():
    items = [
        {"genre": "neutral", "sfx": "none", "speaker": "narrator"},
        {"genre": "horror", "sfx": "none", "speaker": "Priya"},
    ]
