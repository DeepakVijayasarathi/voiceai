...
def health():
    return {
        ...
        "voice_note": "This engine has ONE fixed voice per language - no true multi-speaker model backs it. On /dream-to-story, /stories/{id}/branch, and /characters/{id}/revive, dialogue lines attributed to a known named character get a deterministic per-character treatment on that same base voice (see voice_profile.py): pitch AND vocal-tract formants are shifted INDEPENDENTLY (formants are frequency-warped separately from pitch, not just carried along by a naive pitch shift), plus a shelf-EQ tilt and a slight speaking-pace offset - decoupling formants from pitch is what keeps a shifted voice sounding like a differently-sized speaker instead of the same voice sped up or slowed down. The specific treatment is chosen to fit that character's personality (a menacing villain leans deep with formants pulled down further than pitch alone would, a child character gets both pitch and formants raised, disproportionately) rather than assigned arbitrarily - a real, audible differentiation, but still one underlying voice, not separate speakers.",
...
def config():
    ...
    return {
        ...
        "voice_profiles": {
            ...
            "profiles": {
                name: {
                    "pitch_shift_semitones": v[0], "shelf_eq_tilt_db": v[1],
                    "speed_multiplier": v[2], "formant_ratio": v[3],
                }
